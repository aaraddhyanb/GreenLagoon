...
const char* cs6963::cMainWindow::m_sClassName = "IronMan's Main Window Class";

namespace
{
	// This text will be displayed in the title bar of the main window
	const char* s_windowCaption = "IronMan's CS6963 Renderer";
}
...
HWND cs6963::cMainWindow::GetHandle() const
{
	return m_window;
}
...
bool cs6963::cMainWindow::Initialize( const HINSTANCE i_thisInstanceOfTheApplication,  HWND window )
{
...
int cs6963::cMainWindow::WaitForShutdown() const
{
...
cs6963::cMainWindow::~cMainWindow()
{
	Destroy();
	UnregisterClass();
}
...
bool cs6963::cMainWindow::Create( const int i_initialWindowDisplayState )
{
	// The window's style
	DWORD windowStyle =
		...
		// Change the window's size based on the requested user settings
		{
			...
			// Resize the window
			{
				...
				if ( MoveWindow( m_window, windowCoordinates.left, windowCoordinates.top,
					UserSettings::GetWidth() + nonClientAreaSize.width, UserSettings::GetHeight() + nonClientAreaSize.height,
...
bool cs6963::cMainWindow::Destroy()
{
...
bool cs6963::cMainWindow::RegisterClass()
{
...
bool cs6963::cMainWindow::UnregisterClass()
{
	bool wereThereAnyErrors = false;
...
LRESULT CALLBACK cs6963::cMainWindow::OnMessageReceived( HWND i_window, UINT i_message, WPARAM i_wParam, LPARAM i_lParam )
{
